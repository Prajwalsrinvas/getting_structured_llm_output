...
from outlines.processors.base_logits_processor import (Array,
...
class LogitTrackingProcessor(OutlinesLogitsProcessor):
    ...
    def get_top_tokens(
        ...
        for pos in positions:
            ...
            next_token = (
                self.sequence(pos + 1)[len(text_so_far) :]
                ...
                if top_k is not None and min_value is not None:
                    ...
                    if len(valid_indices) > top_k:
                        valid_indices = valid_indices[
                            np.argsort(max_vals[valid_indices])[-top_k:]
                        ]
...
def template(
    model,
    prompt: str,
    system_prompt: str = "You are a helpful assistant, responding in JSON.",
) -> str:
...
def plot_token_distributions(tracking_processor, k=10, positions=None, prefix=""):
    ...
    for idx, pos in enumerate(positions):
        ...
        for i, (v1, v2) in enumerate(
            zip(unstructured[top_indices], structured[top_indices])
        ):
...
def plot_heatmap(
    ...
    max_probs = np.maximum(
        things["unstructured"].max(axis=1), things["structured"].max(axis=1)
    )
    ...
    unstructured_masked, structured_masked = [
        (x - x.mean(0)) / x.std(0) for x in (unstructured_masked, structured_masked)
    ]
